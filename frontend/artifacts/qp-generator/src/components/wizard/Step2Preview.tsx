...
// These extend the flat wire schema (GeneratedQuestion) with optional richer
// fields. When the backend omits them, we fall back to the flat `text` field.
// ---------------------------------------------------------------------------
...
/** GeneratedQuestion optionally carrying structured sub-questions & figures. */
  ...
  return (
    <div className="max-w-[21cm] mx-auto animate-in fade-in pb-20 print:pb-0">
      ...
      <div className="bg-white p-8 md:p-12 shadow-sm border border-slate-200 print:shadow-none print:border-none print:p-0">
        ...
        {/* Questions Table — table-fixed keeps columns stable; cells auto-expand in height */}
        <table className="w-full table-fixed border-collapse border border-black text-sm text-black mb-8 break-inside-auto">
          <thead>
            <tr>
...
function AutoGrowTextarea({
  ...
  return (
    <textarea
      ...
      rows={1}
      className="w-full bg-transparent resize-none overflow-hidden focus:outline-none hover:bg-slate-50 print:hover:bg-transparent transition-colors leading-snug"
    />
  )
}
    ...
    <tr key={`${q.qNo}-${i}`} className="print-break-inside-avoid">
      ...
      <td className="border border-black p-2 align-top">
        <div className="flex gap-1.5">
          {multi && <span className="font-medium shrink-0 pt-0.5">{sub.label})</span>}
          <div className="flex-1 min-w-0">
            <AutoGrowTextarea
              value={sub.text}
              onChange={v => updateSubText(q.qNo, i, v)}
            />
