...
// ── SegmentEditor: renders math via KaTeX, text as spans. Click to edit. ─────
function SegmentEditor({
  segments,
  rawText,
  onChange,
}: {
  segments?: import("@/components/QuestionRenderer").QuestionSegment[]
  rawText: string
  ...
  if (editing) {
    return (
      <textarea
        ...
        onKeyDown={(e) => { if (e.key === "Escape") setEditing(false) }}
        className="w-full bg-slate-50 border border-slate-300 rounded resize-none focus:outline-none focus:ring-1 focus:ring-blue-400 min-h-[40px] p-1 text-sm font-mono"
        rows={Math.max(1, Math.ceil(rawText.length / 80))}
      />
    )
  }
  ...
  return (
    <div
      onClick={() => setEditing(true)}
      className="w-full min-h-[40px] cursor-text hover:bg-slate-50 rounded p-1 transition-colors print:cursor-default print:hover:bg-transparent leading-relaxed"
      title="Click to edit"
    >
          ...
          if (seg.type === "math") {
            return (
              <KaTeXRenderer
                ...
                onError={() => (
                  <code className="px-1 py-0.5 rounded bg-slate-100 font-mono text-xs text-rose-600">
                    {seg.latex}
                  </code>
                )}
              />
