...
const buildHttpResourceFunction = (verbOption, route, output) => {
	...
	let encodedRoute = output.urlEncodeParameters ? makeRouteSafe(signalRoute) : signalRoute;
...
const generateHttpResourceClient = (verbOptions, options) => {
	...
	return {
		implementation: "\n",
		imports: [...getHttpResourceVerbImports(verbOptions, options.context.output), ...baseUrlOption ? [{
...
const bothClientBuilder = {
	...httpClientBuilder,
	extraFiles: async (verbOptions, output, context) => [...await generateHttpResourceExtraFiles(verbOptions, output, context), ...await generateAngularBaseUrlExtraFiles(verbOptions, output, context)]
};
