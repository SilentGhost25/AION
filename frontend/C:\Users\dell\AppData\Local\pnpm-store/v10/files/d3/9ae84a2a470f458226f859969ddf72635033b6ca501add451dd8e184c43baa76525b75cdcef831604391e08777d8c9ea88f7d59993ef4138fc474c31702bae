...
import { DefaultTag, FormDataArrayHandling, GetterPropType, NamingConvention, OutputClient, OutputHttpClient, OutputMockType, OutputMode, PropertySortOrder, RefComponentSuffix, SupportedFormatter, asyncReduce, buildDynamicScope, buildSchemaTagMap, collectReferencedComponents, compareNatural, conventionName, createSuccessMessage, dynamicImport, escapePath, fixCrossDirectoryImports, fixRegularSchemaImports, generateComponentDefinition, generateDependencyImports, generateMutator, generateParameterDefinition, generateSchemasDefinition, generateVerbsOptions, getBaseUrlRuntimeImports, getConfiguredMockDirectories, getFileInfo, getFullRoute, getImportExtension, getMockFileExtensionByTypeName, getRefInfo, getRoute, isBoolean, isComponentRef, isFunction, isNullish, isObject, isReference, isString, isUrl, jsDoc, kebab, log, logError, logVerbose, logWarning, pascal, removeFilesAndEmptyFolders, resolveInstalledVersions, resolveRef, resolveValue, splitSchemasByType, upath, withGeneratedFileTransform, writeGeneratedFile, writeSchemas, writeSchemasTagsSplit, writeSingleMode, writeSplitMode, writeSplitTagsMode, writeTagsMode, writeTagsOperationsMode, writeTagsOperationsSplitMode } from "@orval/core";
...
function normalizeAngularBaseUrl(baseUrl, outputClient, outputBaseUrl) {
	...
	if (outputClient !== OutputClient.ANGULAR) logWarning(`⚠️  \`override.angular.baseUrl\` is only supported by the \`angular\` client. It has no effect for other clients.`);
	return {
		apiId: baseUrl.apiId,
...
function normalizeOperationsAndTags(operationsOrTags, workspace, global, source) {
	...
	return Object.fromEntries(Object.entries(operationsOrTags).map(([key, { transformer, mutator, formData, formUrlEncoded, paramsSerializer, paramsFilter, query, angular, zod, effect, ...rest }]) => {
		...
		if (unsupportedOperationZodKeys && unsupportedOperationZodKeys.length) {
			...
			logWarning(`⚠️  override.${source}.${key}.zod only supports strict, generate, coerce, preprocess, params, and useBrandedTypes. Ignoring unsupported ${fieldLabel}: ${unsupportedFields}.`);
		}
		...
		if (angular?.baseUrl) logWarning(`⚠️  override.${source}.${key}.angular.baseUrl is not supported — \`baseUrl\` is an output-level concern and is configured via \`override.angular.baseUrl\`. Ignoring.`);
		return [key, {
			...rest,
			...angular ? { angular: {
...
async function createPrettierFileTransform(projectTitle) {
	...
	return async (filePath, content) => {
		try {
			...
			const detail = error instanceof Error ? error.toString() : "unknown error";
			logWarning(`⚠️  ${projectTitle ? `${projectTitle} - ` : ""}Failed to format file ${filePath}: ${detail}`);
			return content;
		}
	};
}
		...
		await Promise.all(filePaths.map(async (filePath) => {
			try {
				...
				if (error instanceof Error) {
					if (error.name === "UndefinedParserError") {} else logWarning(`⚠️  ${projectTitle ? `${projectTitle} - ` : ""}Failed to format file ${filePath}: ${error.toString()}`);
...
async function writeZodSchemas(builder, schemasPath, fileExtension, header, output, paramsMutator, schemaTagMap) {
	...
	for (const generatorSchema of schemasWithOpenApiDef) {
		...
		const filePath = isSplit ? nodePath.join(schemasPath, tagDir, `${fileName}${fileExtension}`) : nodePath.join(schemasPath, `${fileName}${fileExtension}`);
		const context = {
			spec: builder.spec,
			target: builder.target,
			workspace: "",
...
async function runFormatter(formatter, paths, projectTitle) {
	switch (formatter) {
		...
		case SupportedFormatter.BIOME:
			...
			break;
		case SupportedFormatter.OXFMT: await runExternalFormatter(SupportedFormatter.OXFMT, paths, projectTitle);
	}
}
	...
	if (output.schemas) {
		...
		if (isZodSchemas) {
			...
			const schemasParamsMutator = output.override.zod.params ? await generateMutator({
				output: nodePath.join(schemasPath, `__params__${fileExtension}`),
...
async function writeSpecs(builder, workspace, options, projectName) {
	const transform = options.output.formatter === SupportedFormatter.PRETTIER && !options.hooks.afterAllFilesWrite ? await createPrettierFileTransform(projectName ?? builder.info.title) : void 0;
...
async function generateSpec(workspace, options, projectName) {
	if (options.output.clean) {
		...
		if (options.output.schemas) ownedDirectories.add(toComparablePath(isString(options.output.schemas) ? options.output.schemas : options.output.schemas.path));
		...
		for (const directory of mockDirectories) {
			...
			if (mockRootStats.isSymbolicLink()) {
				logWarning(`${projectName ? `${projectName} ` : ""}Skipped cleaning "${directory}": the configured mock path is a symbolic link.`);
				continue;
			}
...
//# sourceMappingURL=config-BzEy9MjR.mjs.map