...
function getEnumImplementation(members, options) {
	...
	for (const member of membersWithoutNull) {
		...
		const rawKey = member.name ? member.name : deriveEnumKey(member.value, options.enumNamingConvention, disambiguate);
...
function getEnumMembersFromBranches(schemaObject) {
	if (!schemaObject) return [];
...
function resolveObjectOriginal({ schema, propName, combined = false, context, formDataContext }) {
	...
	const isSchemaNullableAtRoot = (schema) => {
	...
	if (propName && resolvedValue.isEnum && !combined && !resolvedValue.isRef) {
		...
		const enumValue = getEnum(getEnumMembers(resolvedValue.originalSchema), propName, isSchemaNullableAtRoot(resolvedValue.originalSchema), context.output.override.enumGenerationType, context.output.override.namingConvention.enum);
		return {
			value: propName,
...
function resolveServerUrl(servers, options) {
	...
	for (const variableKey of Object.keys(server.variables)) {
		...
		if (variables?.[variableKey] !== void 0) {
...
function getFullRoute(route, servers, baseUrl) {
	const getBaseUrl = () => {
		...
		if (baseUrl.getBaseUrlFromSpecification) {
			...
			return esc(resolveServerUrl(servers, {
				index: baseUrl.index,
...
function generateSchemaDefinitions(schemaName, schema, context, suffix) {
	...
	output += jsDoc(schema);
	if (resolvedValue.isEnum && !resolvedValue.isRef) output += getEnum(getEnumMembers(resolvedValue.originalSchema), sanitizedSchemaName, !!nullable, context.output.override.enumGenerationType, context.output.override.namingConvention.enum);
