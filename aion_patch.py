...
def get_module_partition(mod_idx=1):
    ...
    if mod_idx in _MODULE_PARTITIONS and _MODULE_PARTITIONS[mod_idx]:
...
def collect_extracted_assets(file_path: str):
    """Return real equation latex + image files next to an uploaded PDF."""
    from pathlib import Path as _P
    import json as _j, re as _re
    pdf = _P(file_path)
    upload_dir = pdf.parent if pdf.suffix.lower() == ".pdf" else _P(file_path)
    images = []
    for ext in ("*.png", "*.jpg", "*.jpeg", "*.webp"):
        images.extend(str(x) for x in upload_dir.rglob(ext) if x.is_file() and x.stat().st_size > 2000)
    # skip tiny icons
    images = sorted(set(images))
    equations = []
    for md in list(upload_dir.rglob("*.md"))[:8]:
        try:
            txt = md.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            continue
        equations += _re.findall(r"\$\$(.+?)\$\$", txt, _re.S)
        equations += _re.findall(r"\\begin\{equation\*?\}(.+?)\\end\{equation\*?\}", txt, _re.S)
    equations = [e.strip() for e in equations if e.strip()][:30]
    return {"images": images[:24], "equations": equations}
